...
function PM:CreateUI()
    ...
    if self.settings.ui.left == self.defaults.ui.left and self.settings.ui.top == self.defaults.ui.top then
        -- Snap to the right of the Compass by default
        ui:SetAnchor(LEFT, ZO_Compass, RIGHT, 25, 0)
    else
        -- Use users saved position
    ...
    ui:SetHandler("OnUpdate", function()
        -- Only update if theres an active memento
...
function PM:Init(eventCode, addOnName)
    ...
    SLASH_COMMANDS["/pmem"] = function(extra)
        ...
        elseif cmd == "uireset" then
            ...
            self:Log("UI Position Reset to Compass.")
        ...
        elseif cmd == "list" or cmd == "" then
            ...
            d("/pmem uireset - Reset UI to Compass")
            d("--------------------------------")
        ...
        elseif cmd == "current" then
            ...
            for id, info in pairs(self.mementoData) do
                if string.find(string.lower(info.name), cmd, 1, true) then
                    self.settings.activeId = id
                    self.settings.paused = false
                    UseCollectible(id) 
                    self:Log("Started: " .. info.name)
                    zo_callLater(function() self:Loop() end, info.dur)
                    found = true
                    break
                end
            end
            if not found then
                self:Log("Memento not found.")
            end
        end
    end
